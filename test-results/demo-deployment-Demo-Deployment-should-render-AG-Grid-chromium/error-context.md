...
- Name: Demo Deployment >> should render AG Grid
- Location: /Users/ryan/code-repos/github/ryanrozich/ag-grid-react-components/tests/e2e/demo-deployment.spec.ts:55:3
...
Call log:
  - waiting for locator('.ag-root-wrapper') to be visible
    ...
    at /Users/ryan/code-repos/github/ryanrozich/ag-grid-react-components/tests/e2e/demo-deployment.spec.ts:57:16
```
...
>  57 |     await page.waitForSelector(".ag-root-wrapper");
  ...
  131 |         expect(href).toMatch(
  132 |           new RegExp(`^${expectedBasePath.replace(/\//g, "\\/")}assets`),
  133 |         );
  134 |       }
  135 |     }
  136 |
  137 |     for (const script of scriptTags) {
  138 |       const src = await script.getAttribute("src");
  139 |       if (src && !src.startsWith("http")) {
  140 |         expect(src).toMatch(
  141 |           new RegExp(`^${expectedBasePath.replace(/\//g, "\\/")}assets`),
  142 |         );
  143 |       }
  144 |     }
  145 |   });
