...
- Location: /Users/ryan/code-repos/github/ryanrozich/ag-grid-datefilter-fix/tests/e2e/server-side-demo-fixed.spec.ts:55:3

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: locator('.ag-header-row') resolved to 2 elements:
    1) <div role="row" aria-rowindex="1" class="ag-header-row ag-header-row-column">…</div> aka getByRole('row', { name: 'ID', exact: true })
    2) <div role="row" aria-rowindex="1" class="ag-header-row ag-header-row-column">…</div> aka getByRole('row', { name: 'Task Status Priority Category' })

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('.ag-header-row')

    at /Users/ryan/code-repos/github/ryanrozich/ag-grid-datefilter-fix/tests/e2e/server-side-demo-fixed.spec.ts:64:50
```
