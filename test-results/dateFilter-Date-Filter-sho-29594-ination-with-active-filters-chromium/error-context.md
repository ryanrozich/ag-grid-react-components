...
- Location: /Users/ryan/code-repos/ag-grid-react-components/tests/e2e/dateFilter.spec.ts:74:3
...
>  74 |   test("should handle pagination with active filters", async ({ page }) => {
  ...
  168 |
  169 |   for (const dateStr of dates) {
  170 |     const rowDate = new Date(dateStr);
  171 |
  172 |     switch (filterType) {
  173 |       case "equals":
  174 |         expect(rowDate.toDateString()).toBe(filterDate.toDateString());
```
