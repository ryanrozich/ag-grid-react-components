...
- Name: Avatar Loading >> should use consistent avatars for same names
- Location: /Users/ryan/code-repos/ag-grid-react-components/tests/e2e/avatars.spec.ts:96:3
...
>  96 |   test("should use consistent avatars for same names", async ({ page }) => {
  ...
  111 |       }
  112 |
  113 |       // All avatars for the same person should have the same URL
  114 |       const uniqueSrcs = new Set(avatarSrcs);
  115 |       expect(uniqueSrcs.size).toBe(1);
  116 |     }
  117 |   });
  118 | });
  119 |
